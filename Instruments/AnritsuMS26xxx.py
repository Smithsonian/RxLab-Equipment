...
class SpecA:
  def __init__(self, InstAddr="GPIB::2"):
    ...
    self.freq_span = self.speca.query_values("SP?")[0]/1.0e9
    self.freq_centre = self.speca.query_values("CF?")[0]/1.0e9
    self.freq_start = self.speca.query_values("FA?")[0]/1.0e9
    self.freq_stop = self.speca.query_values("FB?")[0]/1.0e9
    self.freq_step = (self.freq_stop-self.freq_start)/500.0
    self.rbw = self.speca.query_values("RB?")[0]
    self.vbw = self.speca.query_values("VB?")[0]
  ...
  def get_trace(self):
    ...
    trace_data = self.speca.query_values("XMA? 0,501")
