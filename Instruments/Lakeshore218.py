...
class Lakeshore(object):
  ...
  def get_temp_log(self,outfilename,sensor_list):
    ...
    for i in range(1,last_log+1):
        ...
        output_string=""
        log_str=self.visa_query("LOGVIEW? {:d} 1".format(i))
        log_list=log_str.split(",")
        output_string += (log_list[0]+" "+log_list[1])
        for j in sensor_list:
          log_str=self.visa_query("LOGVIEW? {:d} {:d}".format(i,j))
          log_list=log_str.split(",")
  ...
  def get_logvalue(self, record, reading):
    ...
    retval = self.visa_query("LOGVIEW? {:d} {:d}".format(record, reading))
  ...
  def get_logdatetime(self, record):
    ...
    retval = self.visa_query("LOGVIEW? {:d} {:d}".format(record, 1))
  ...
  def get_lognum(self):
    ...
    return int(self.visa_query("LOGNUM?"))
  ...
  def get_logunits(self, reading):
    ...
    retval = self.visa_query("LOGREAD? {:d}".format(reading))
  ...
  def get_temp(self, sensor, unit="K"):
    ...
    if unit=="K": # get reading in Kelvin
        reading = self.visa_query("KRDG? {:d}".format(sensor))

    elif unit=="C": # get reading in Celsius
        reading = self.visa_query("CRDG? {:d}".format(sensor))

    elif unit=="L": # get reading in Linear number
        reading = self.visa_query("LRDG? {:d}".format(sensor))
    else: # get reading in Sensor units
        reading = self.visa_query("SRDG? {:d}".format(sensor))
  ...
  def sensor_enabled(self, sensor):
    ...
    onoff = self.visa_query("INPUT? {:d}".format(sensor))
  ...
  def sensor_calibrated(self, sensor):
      ...
      calibrated = self.visa_query("INCRV? {:d}".format(sensor))
