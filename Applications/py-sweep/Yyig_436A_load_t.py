...
revision 3/14/2012 always put ambload out first
...
def Tmeasure():
    ...
    print 'This gives a measured temp of '+ str(Tdata) + 'K. Actual temp used is {.1f}' + str(Treg)
    return Treg
...
if len(sys.argv) >= 8:
    ...
    Tamb = float(sys.argv[7])
    Tcold = float(sys.argv[6])
else:
...
f.write("Tamb = %.1f\t Tcold = %.1f" %(Tamb,Tcold))
